...
1. `main_training.m`
   ...
   - Calls `model_training.m` for specific model training
...
### 7. How is training convergence ensured?
...
1. 可能无法完全捕捉生产过程中的时间耦合
2. 模型放松了整数变量,在某些场景下可能影响准确性
3. 使用了简化的工业过程表示
4. 总能耗分解到各个设备需要额外考虑
...
1. 谨慎选择学习率(例如,0.05而不是0.9)
2. 实施不同的迭代策略(单日或多日批量训练)
3. 监控训练过程中的关键参数直到稳定(相对变化<0.02%)
4. 使用基于参数收敛的适当停止标准
